...
def prob_plot(num_decompositions=25) -> dict[str, list[ProbPlotRecord]]:
    ...
    res["e2_psi_4"] = optimize_expr(
        make_expr_th2(all_states["psi_4"], expected_extra_photons["psi_4"])
    )
    res["e1_psi_8"] = optimize_expr(make_expr_th1(all_states["psi_8"]))
    res["e2_psi_8"] = optimize_expr(
        make_expr_th2(all_states["psi_8"], expected_extra_photons["psi_8"])
    )
    res["e1_psi_9"] = optimize_expr(make_expr_th1(all_states["psi_9"]))
    res["e2_psi_9"] = optimize_expr(
        make_expr_th2(all_states["psi_9"], expected_extra_photons["psi_9"])
    )
